...
export class BudgetEffects {
    ...
    createBudget = this.actions.pipe(
        ...
        switchMap((budget: Budget) => this.http.post<Budget>(this.budgetEndPoint, budget).pipe(
                mergeMap((bud: Budget) => [
                        {
    ...
    updateBudget = this.actions.pipe(
        ...
        switchMap((budget: Budget) => this.http.post<Budget>(this.budgetEndPoint.concat('/update'), budget).pipe(
                mergeMap((bud: Budget) => [
                        {
    ...
    deleteBudget = this.actions.pipe(
        ...
        map((action: BudgetActions.DeleteBudget) => action.payload),
        switchMap((id: number) => this.http.delete(this.budgetEndPoint.concat('/').concat(id.toString())).pipe(
                mergeMap((budgetId: number) => [
                        {
    ...
    listBudgets = this.actions.pipe(
        ...
        switchMap((action: BudgetActions.ListBudgets) => this.http.get(this.budgetEndPoint, {
