...
export class EditSavingsComponent implements OnInit {
    ...
    ngOnInit() {
        this.store.select(categories).subscribe((catgs: Category[]) => {
            this.categories = catgs.filter((cat) => cat.savings);
        });
        this.initForm();
    }
    ...
    private createSavings(createdDate: any) {
        ...
        if(this.savingsForm.get('operation').value === this.IN) {
            const cats = this.categories.filter((cat) => cat.id === +this.savingsForm.get('category').value);
            this.store.dispatch(new CreateExpense(new Expense(-1, this.savingsForm.get('description').value, cats[0], loggedUser,
                this.savingsForm.get('value').value, createdDate, this.translate.instant('Automatically created by savings'))));
        } else {
            value *= -1;
            this.store.dispatch(new CreateIncome(new Income(-1, loggedUser, this.savingsForm.get('description').value,
                this.savingsForm.get('value').value, createdDate, this.translate.instant('Automatically created by savings'))));
        }
        this.store.dispatch(new CreateSavings(new Savings(-1, loggedUser, this.savingsForm.get('description').value,
            value, createdDate)));
    }
    ...
    initForm() {
        ...
        if (this.currentId > 0) {
            ...
            this.store.select(savings).subscribe((savs: Savings[]) => {
                const newSavings = savs.find((sav: Savings) => sav.id === this.currentId);
                if (newSavings) {
                    description = newSavings.description;
                    value = newSavings.value;
                    createdDate = newSavings.createdDate;
                }
            });
        }
