...
export function savingsReducers(state = initialSavingsState, action: SavingsActions): SavingsState {
    switch (action.type) {
        ...
        case (SavingsActionsEnum.ADD_SAVINGSS): {
            return {
                ...state,
                savings: [...state.savings, ...action.payload.content]
                    .sort((sav1, sav2) => new Date(sav2.date).getTime() - new Date(sav1.date).getTime()),
                page: action.payload
            };
        }
        case (SavingsActionsEnum.ADD_SAVINGS): {
            ...
            return {
                ...state,
                savings: [action.payload, ...newSavingss]
                    .sort((sav1, sav2) => new Date(sav2.createdDate).getTime() - new Date(sav1.createdDate).getTime())
            };
        }
